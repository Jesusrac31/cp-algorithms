...
lli soluciones(lli a, lli b, lli x, lli y, lli d, lli p){
    if (p%d != 0){
        return 0;
    }
    x*=p/d;
    ...
    long double lim_up, lim_down;
    lim_down = ceil((long double)(-x*d)/b);
    lim_up = floor((long double)(y*d)/a);

    return relu(lim_up-lim_down+1);
}
...
int solve() {
    ...
    for(int i = 0;p-c*i >= 0; i++){
        sol+=soluciones(a,b,x,y,d,p-c*i);
    }
