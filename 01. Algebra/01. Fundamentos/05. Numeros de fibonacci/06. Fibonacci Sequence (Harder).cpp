...
const int MOD = 1e9+7;
const int periodo = 2e9+16; // Cada periodo números de fibonacci, se repite la secuencia módulo MOD
...
pair<Mint, Mint> fibPair(unsigned long long n) {
    if (n == 0) return {0, 1};
    auto p = fibPair(n >> 1);
    Mint a = p.first;
    Mint b = p.second;
...
Mint getFibo(unsigned long long n) {
